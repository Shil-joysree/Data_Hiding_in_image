...
def encrypt_message(img_path):
    ...
    img = np.array(img, dtype=np.uint8)  # Ensure values remain within uint8 range
...
def decrypt_message():
    ...
    for char in binary_chars:
        if len(char) == 8:
            ...
            if decoded_char == '\x00':  # Prevents unintended null characters
                break
    ...
    if password.strip() != entered_pass.strip():
...
def main():
    img_path = r"C:\Users\Lenovo\Downloads\Data-Hiding-In-Image-main\Data-Hiding-In-Image-main\cybersecurity.jpg"  # Input image
